...
func testCandidateEquality(t *testing.T) {
	...
	adopted, err := client.ValidateCandidate(context.Background(), plaintext, candidate, "app/token")
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func testCandidateMismatch(t *testing.T) {
	...
	adopted, err := client.ValidateCandidate(context.Background(), []byte("original-plaintext"), []byte(`{"data":"x"}`), "app/token")
...
func testCandidateEmptyPlaintext(t *testing.T) {
	...
	adopted, err := client.ValidateCandidate(context.Background(), nil, candidate, "app/token")
	if err != nil {
		t.Fatalf("err = %v", err)
	}
	if !bytes.Equal(adopted, candidate) {
...
func testCandidateEmpty(t *testing.T) {
	...
	adopted, err := client.ValidateCandidate(context.Background(), []byte("p"), nil, "app/token")
	expectKind(t, err, failure.Operational)
	if adopted != nil {
...
func testCandidateMalformed(t *testing.T) {
	...
	repository := t.TempDir()
	client, _ := newTestClient(t, clientTarget{executable: executable, repository: repository, behavior: sops.Behavior{ExitCode: 9}})
	adopted, err := client.ValidateCandidate(context.Background(), []byte("p"), []byte("not-json"), "app/token")
	expectKind(t, err, failure.Operational)
	if adopted != nil {
...
func testCandidatePartial(t *testing.T) {
	...
	adopted, err := client.ValidateCandidate(context.Background(), plaintext, []byte(`{"data":"x"}`), "app/token")
	expectKind(t, err, failure.Operational)
	if adopted != nil {
...
func testCandidateFailure(t *testing.T) {
	...
	adopted, err := client.ValidateCandidate(context.Background(), []byte("p"), []byte(`{"data":"x"}`), "app/token")
	expectKind(t, err, failure.Operational)
	if adopted != nil {
